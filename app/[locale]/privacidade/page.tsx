...
export default async function Privacy({ params }: PageProps<"/[locale]/privacidade">) {
  ...
  return (
    <>
      ...
      <main id="main" tabIndex={-1} data-theme="paper" className="section relative z-10 pt-[calc(var(--header-h)+var(--space-section))]">
        <article className="container-content">
          <div className="max-w-[68ch]">
            <h1 className="font-serif text-display-md font-light">{t("title")}</h1>
            {isSet(site.privacy.updated) && <p className="mt-4 text-body-sm text-fg-subtle">{t("updated", { date: site.privacy.updated })}</p>}
            <nav aria-labelledby="toc-title" className="mt-10 border-y border-line py-6">
              <h2 id="toc-title" className="hud">{t("toc")}</h2>
            ...
            {ids.map((id) => (
              <section key={id} aria-labelledby={id} className="mt-10">
                <h2 id={id} className="font-serif text-heading-md font-light">{t(`sections.${id}.title`)}</h2>
